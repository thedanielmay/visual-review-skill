...
- Report sections: HOLD, FIX IF TIME, NOTE, BEYOND VISUAL, SUPPRESSED FINDINGS, MECHANICAL FIXES (skipped); STACK only if triggered
...
- Report sections: all sections including CROSS-PAGE DRIFT, TYPE-SPECIFIC CHECKS; STACK only if triggered
...
```
0. Precondition gate  →  1. Identify input  →  1.5. Stack assessment (always)
                                    ↓
                         2. Render + capture metadata
                                    ↓
...
**Renderer reference — load unconditionally:**

Read `references/pagedjs.md` now, for any PDF or HTML input. The file is ~200 lines and contains renderer-specific checks (PJ-1 through PJ-7) that are additive to the universal checks in Phase 3. Apply PJ-checks only when Paged.js signals are confirmed in Phase 2 metadata; skip them otherwise. Loading early avoids the detection-too-late problem (PDF metadata isn't available until Phase 2).
...
### Phase 1.5 — Stack detection (always runs silently; STACK section only on trigger)

Stack detection runs in every mode, every review. It takes under a minute. **The STACK section is only emitted in the Phase 8 report when at least one trigger fires.** A clean review with a healthy stack produces no STACK section — silence means nothing concerning was detected.

#### Stack detection (run these checks silently)

```bash
# PDF metadata — most reliable signal
pdfinfo "$INPUT_PDF" 2>/dev/null | grep -E "Producer|Creator"

# Node ecosystem
ls package.json 2>/dev/null && grep -E '"pagedjs|puppeteer|playwright|react-pdf|pdf-lib|jspdf"' package.json

# Python ecosystem
grep -E "weasyprint|reportlab|fpdf|pdfkit" requirements.txt pyproject.toml setup.py 2>/dev/null

# HTML source signals
grep -l "pagedjs\|paged\.polyfill\|\.pagedjs_page" *.html index.html 2>/dev/null

# LaTeX/Typst
ls *.tex *.typ 2>/dev/null
```

Corroborate signals. See `visual-stack-review/SKILL.md` detection caveats — monorepos, CDN-loaded tools, and dead deps all cause misclassification. Do not label confidence above `possible` from a single signal.

#### Trigger conditions — emit STACK section when ANY of these fire:

**Trigger 1 — Unmaintained or CVE-carrying stack (always HOLD-level)**
Detected stack is wkhtmltopdf, or any tool with known unpatched CVEs. Do not present "stay + fix." Emit a migration brief directly.

```
### HOLD — Stack: [wkhtmltopdf | other unmaintained tool] detected
Signals: [list]
This tool is unmaintained / has known CVEs. Do not continue using it in a server-side
rendering pipeline. See /visual-stack-review for migration options.
```

**Trigger 2 — Phase 3 finding traceable to a known stack bug**
One or more findings from the visual pass are caused by a documented systematic bug in the detected stack (e.g. Paged.js PJ-3 orphan border-top, WeasyPrint pre-v60 SVG mis-render). Surface under STACK in the report, linking finding to root cause.

```
## Stack: [Tool name] (signals: [list])

### Findings traceable to this stack
- [Finding ID] on page N — root cause: [stack bug name/reference]
  Fix available: [CSS workaround / upgrade to version X / structural limitation]

→ Reply "stack alternatives" for a migration comparison via /visual-stack-review
```

**Trigger 3 — User explicitly asks**
User asks "what stack is this?", "is this the right tool?", "should I switch?", or similar. Then emit the full stay + fix assessment from /visual-stack-review Phase 3.

#### No trigger fires → no STACK section

If none of the three triggers fire, omit the STACK section from the Phase 8 report entirely. Do not add a boilerplate note. Silence is the correct output for a healthy stack.
...
**Zone-crop checks (mandatory — full-page view misses 1–4px defects).**

**When shell access is available,** generate crops programmatically after the full-page render:

```bash
# After pdftoppm produces page-NN.png files at 300 DPI:
# A4 portrait at 300 DPI = 2480×3508px. A4 landscape = 3508×2480px.
# Adjust PAGE_W and PAGE_H for your page size.

PAGE_W=3508   # A4 landscape width in px at 300 DPI
PAGE_H=2480   # A4 landscape height in px at 300 DPI

mkdir -p crops
for PAGE in output_dir/page-*.png; do
  BASE=$(basename "$PAGE" .png)
  # Top zone — header clearance and orphan border (top 15mm = 177px at 300 DPI)
  convert "$PAGE" -crop "${PAGE_W}x177+0+0" +repage "crops/${BASE}-top.png"
  # Bottom zone — footer clearance and last-line completion (bottom 10mm = 118px)
  convert "$PAGE" -crop "${PAGE_W}x118+0+$((PAGE_H - 118))" +repage "crops/${BASE}-bottom.png"
done
echo "Zone crops saved to crops/"
```

Inspect each crop using the Read tool (it can display images). Include the crop filename in any finding that references that zone.

**If ImageMagick (`convert`) is not available,** fall back to verbal zone examination:

```bash
which convert && echo "ImageMagick available" || echo "ImageMagick not installed — using verbal fallback"
```

For each page at 300 DPI render, explicitly examine these zones as separate mental passes — do not rely on the full-page gestalt:

1. **Top 15mm of content area** — any horizontal rule that is NOT the header rule is an orphan `border-top`
2. **Bottom 10mm of content area** — body text whose last line sits in the same horizontal band as footer text
3. **Column gutter** — on two-column pages, confirm both columns have content
4. **Rule-containing pages** — zoom into each rule and confirm it does not overprint glyphs

**300 DPI is non-negotiable.** Previous reviews at lower DPI missed orphan border-tops (~1–2px), footer overprint (two text layers merge at low DPI), and clipped callout box bottom borders.
...
### Phase 3 — Visual pass (Quick Pass + Full Review)

Phase 3 has two sub-phases run in sequence:
- **3A — Page-level pass**: Things only visible on one page at a time (content clipping, font fallback, overlaps, image quality, typography)
- **3B — Batch comparison pass**: Things requiring comparison across pages (density balance, baseline rhythm, margin drift). Run after all pages have been through 3A.

Quick Pass runs both 3A and 3B (the batch comparison is lightweight — 3 metrics only).
Full Review runs both and feeds into Phase 4 (cross-page consistency) for deeper analysis.

**IMPORTANT — output structure:** The 3A/3B split governs the model's *processing sequence*, not the report structure. Phase 8 output remains a single unified findings list (HOLD / FIX IF TIME / NOTE). Do NOT label findings with "3A" or "3B" in the report — the user should see one coherent list, not two separate sections to mentally stitch together.

#### Phase 3A — Per-page pass
...
3b. **Column top alignment check (mandatory on section opener pages with a spanning header element).** On any page where a full-width spanning element (section title, chapter header, or equivalent) is followed by two-column body content: do both columns begin at the same vertical position? The left column's first content element should share the same approximate top edge as the right column's first content element. If the left column content begins noticeably lower — with a visible gap above it while the right column starts flush — this is a **Tier 2 (FIX IF TIME)** finding on a standard document, **HOLD** on client-facing. Detection: mentally draw a horizontal line at the top of the right column's first content element; the left column's first element should be within one line-height of that line. Root cause: a CSS `padding-top` on the page container that applies before the spanning element, shifting it down; the left column then begins below the span while the right column begins from a mid-flow continuation. Fix: move the padding-top from the page container to the spanning element itself, so the spacing is absorbed by the element that actually needs it.
...
5. **Footer content clearance test** (mandatory on every body page with a footer). Does the body content complete cleanly ABOVE the footer zone, or does it appear to be cut off by the footer strip? Check the last 10% of page height for content that ends abruptly (no terminal punctuation, no visual close, sentence trailing off). If content appears to be clipped by the footer zone — text running into the footer area or being hidden behind it — this is a HOLD finding. **Root cause pattern:** `max-height` set to the full page height (e.g. 170mm on an A4 landscape page) without subtracting the footer strip height (~8mm). Content fills to 170mm but the footer occupies the bottom 8mm, silently clipping the last line(s). **Detection:** look at the gap between the last visible content element and the footer rule line. If there is no visible gap (content appears to touch or overlap the footer), it is clipping. **Fix:** reduce `max-height` to leave footer clearance (e.g. 162mm instead of 170mm), or increase `margin-bottom` in the `@page` rule to give the footer margin box more room.
   ...
   **Footer breathing-room test (distinct from clipping).** Even when content is NOT clipped — the last line ends with terminal punctuation and is technically above the footer — check whether the visual gap between the last content line and the footer rule is sufficient. If the last line of body text and the footer rule appear to share the same visual band (the eye moves from text directly to the rule with no felt gap), this is a **Tier 2 (FIX IF TIME)** finding — HOLD on client-facing. Detection: ask "could a reader's eye comfortably land on the last line before encountering the footer?" If the answer is no — if the rule appears to immediately follow the last line — the clearance is insufficient. This is separate from clipping. Content can complete correctly and still lack breathing room. Fix: increase `@page margin-bottom` by 2–3mm, or add `padding-bottom` to the page content container.

   **Confidence: `likely`** — breathing room involves judgment about visual comfort, not pixel measurement.
...
6. **Header content clearance test** (mandatory on every body page with a running header). Is there a visible breathing gap between the bottom of the running header rule line and the first line of body content below it? Content that starts immediately adjacent to the rule makes the page feel cramped and is a **Tier 2 finding (FIX IF TIME)** — promote to HOLD on client-facing documents.
   ...
   **Renderer-agnostic generalisation (applies to WeasyPrint, Paged.js, Prince, Chrome/Puppeteer).** This check is not specific to any renderer — apply it whenever any horizontal rule appears on the page: a running header rule, a section divider rule, a `.section-head` border-bottom, or a column separator. Ask for each rule: is there visible breathing room between the rule line and the first content element below it? "Breathing room" means you could slide a finger between them and feel space — if the rule and the first content baseline appear to be in the same visual band, the clearance is insufficient.

   - **Section-internal divider rules** (e.g. a `border-bottom` on a heading block, or a `.section-head` separator): if the first paragraph below the rule has no visible top margin and the rule and paragraph baseline appear merged, flag as **FIX IF TIME** (promote to HOLD on client-facing documents).
   - **Running header rules on continuation pages** (pages beginning mid-flow): if the first content element on the page — whether a mid-sentence continuation paragraph, a subheading, or body text — sits flush against the header rule with no visible gap, flag as **FIX IF TIME** (promote to HOLD on client-facing).
   - **Root cause pattern:** a `border-bottom` on a container element combined with `margin-bottom` on that container AND `margin-top: 0` (or no `margin-top`) on the first sibling below it — the gap between rule and content is controlled entirely by the container's `margin-bottom`, which is typically set for block-spacing rather than visual breathing room below a rule.
   - **Fix options:** (a) add `padding-top` (~6–8pt minimum) to the first element after the ruled container, or (b) increase the ruled container's `margin-bottom` until the gap is visually comfortable.
   ...
   **Confidence: `likely`** — "slide a finger" is a judgment call; promote to `confident` only when rule and content baseline share the same visual band (zero visible gap).
   ...
   **What to look at:** There are two separate text elements near the header rule — one above it (the running header text, e.g. "Section 01 · Introduction · May 2026") and one below it (the first body content, e.g. "SECTION 01 · INTRODUCTION"). The clearance finding is exclusively about the gap BELOW the rule, between the rule and the first body content. The running header text above the rule is expected to sit close to it — that is correct behaviour, not a finding.
   ...
   > **[Paged.js: universal header clearance fix → references/pagedjs.md PJ-2]**
   ...
   **Confidence: `likely`** — the 2× threshold is a guideline; flag as `confident` only when the gap is obviously disproportionate (3× or more) relative to structural siblings.
...
7. **Major section opener page-break test** (mandatory on every page that ends with — or begins within the bottom 30% of — a section heading, eyebrow + H1, or numbered section title). Does a major section opener (a new top-level numbered section: "Section 05", "Chapter 3", "Part II", etc.) start on a fresh page? A major section heading appearing in the bottom 30% of any page is a **HOLD finding** — it signals a missing `break-before: page` on the section wrapper.
   ...
   **The failure mode that hides this:** The review focusses on the *following* page (which appears clean — content starting at the top). The *previous* page, which ends with the orphaned heading, reads as "content then whitespace then heading" — superficially similar to an intended design choice. It is not a design choice. A major section opener at the bottom of a page is always a structural break failure.
   ...
   > **[Paged.js: section opener page-break fix → references/pagedjs.md PJ-5]**
   ...
   **Detection:** A thin rule sitting in the gap between the header rule and the first body text, with no eyebrow/label between the rule and the body text. Structure: [header rule] → [6mm gap] → [orphan rule ← THIS] → [body text]. A legitimate Q-section separator always has the separator above an eyebrow label and heading; an orphan border appears above body text with no label.
   ...
   > **[Paged.js: orphan border-top fix → references/pagedjs.md PJ-3]**
...
8. **Flowing section footer collision test** (mandatory on every page whose content comes from a `section-start` without `.page`, i.e. a flowing section). Does the last visible element on the page clear the footer rule with visible breathing room? Any element — a heading, eyebrow label, sentence fragment, or Q-section heading — that visually overlaps or sits inside the footer margin band is a **HOLD finding**.
   ...
   **Detection:** on any page with a flowing section, look at the bottom 8–10% of the page height. Is there a content element (heading, eyebrow, sentence) below the last clear body content gap, sitting immediately above or overlapping the footer text? If yes, it is a footer collision. The tell is seeing the document footer text ("SAFELEDGER · ANALYTICAL DOCUMENT V7 · CONSOLIDATED RECORD") and a body element on the same or adjacent horizontal band.
   ...
   > **[Paged.js: flowing section footer collision fix → references/pagedjs.md PJ-4]**
...
**Confidence for checks 1–4, 7–8: `confident`** — binary observable checks (terminal punctuation, box border presence, column content, footer visibility, table row counts). Checks 5 (footer breathing room), 6 (header clearance calibration), and the upper-bound gap sub-check use `likely` — see individual labels above. If you cannot determine the answer from the render, increase DPI to 300 and re-render before proceeding.
...
> **[Paged.js: long-form continuation headers/footers → references/pagedjs.md PJ-6]**
- **Long-form body text using narrow single-column width — right margin wasted (Tier 2).**
> **[Paged.js: long-form narrow column → references/pagedjs.md PJ-7]**
...
> **[Paged.js: coloured-page background fix → references/pagedjs.md PJ-1]**
...
- **Callout box internal padding asymmetry — heading label floating away from the top edge.** Applies to any styled callout, alert, or aside box (cream background, gold/coloured left-border accent, or similar panel treatment). **Visual symptom:** the heading label inside the box sits noticeably far from the box's top edge, while the body text sits close to (or nearly touching) the box's bottom edge. The box looks bottom-heavy — more air at the top than at the bottom. **Detection:** compare the visual gap between (a) box top border → first content element (heading label), and (b) last content element (body text) → box bottom border. If gap A is more than ~1.5× gap B, flag as asymmetric internal padding. **Root cause:** heading elements inside callout boxes inherit their own `margin-top` value, which stacks on top of the callout's `padding-top`, creating a larger effective top gap than the bottom. Example: `.doc-h3 { margin: 12pt 0 4pt }` inside `.callout { padding: 9pt 12pt }` produces an effective top gap of 21pt (9pt padding + 12pt margin) vs a bottom gap of just 9pt (padding only) — more than 2× asymmetry. **Fix:** add a `:first-child` selector targeting heading elements inside callout containers to zero out the top margin:
  ```css
  .callout .doc-h3:first-child,
  .callout-dark .doc-h3:first-child,
  .callout-risk .doc-h3:first-child { margin-top: 0; }
  ```
  This ensures only the callout's own `padding-top` controls the internal top gap. **Confidence: `likely`** — the asymmetry is visually clear but whether it was intentional requires user confirmation. **Action: FIX IF TIME** (promote to HOLD on client-facing documents).
...
#### Phase 3B — Batch comparison pass (Quick Pass + Full Review)

After completing 3A on all pages, perform this lightweight cross-page scan:

**Metric 1 — Bottom dead-whitespace gap.** For each body page, estimate the gap between
the last content element and the footer rule. Note it. Compute the visual median.
- Pages with gap > 2× median: `excessive whitespace` (Tier 3, or Tier 2 if paired with small text)
- Pages with gap < 0.5× median: `cramping` (Tier 2)
- Flag both directions — asymmetry is the finding.

**Metric 2 — Footer y-coordinate.** Note the footer rule y-position on each body page.
Any page where the footer sits noticeably higher or lower than its neighbours is Tier 3 drift.

**Metric 3 — Body text size.** Spot-check one paragraph per page. Any page where body
text visibly differs in size from its neighbours is Tier 2 drift.

These three metrics are the Quick Pass batch comparison. Full Review Phase 4 expands this
into a complete cross-page measurement pass.

#### Density imbalance — detailed analysis (Phase 3B)
...
**Confidence: `likely`** — median calculation involves eyeballing; flag as `confident` only when the dead-whitespace gap exceeds 30% of page height.
...
#### Finding consolidation (mandatory before Phase 8)

Before writing the Phase 8 report, group findings by root cause:

1. **Identify systematic findings** — any finding that appears on 3+ pages with the same visual description and the same likely root cause (e.g., "header clearance too tight" on pages 3, 7, 11, 15).

2. **Merge into one grouped finding** with:
   - A `Pages affected: N, M, O, P` field
   - One shared repair instruction
   - A per-page sub-note for each page that has a local variation: `p7: adjacent caption present — verify fix doesn't clip it`
   - Sub-notes are required whenever one page's context differs from the others (adjacent elements, different content density, different column layout). When all pages are truly identical in context, sub-notes may be omitted.

3. **Keep separate** any finding where the visual symptom is the same but the root cause differs per page (e.g., clipping on page 3 from `overflow: hidden` and clipping on page 9 from a different `max-height` constraint — these need separate source pointers).

4. **One-page-specific findings** do not consolidate — leave them as individual entries.

5. **Do not lose granularity to achieve cleanliness.** A grouped finding with 4 one-line sub-notes is better than a collapsed finding that hides an outlier the reviewer would have caught individually.

Consolidation reduces report noise: 20 individual "header clearance tight" entries → one grouped entry with sub-notes. The user reads one fix recipe, but retains visibility into each page's specific context.
...
**STACK (conditional — only when Phase 1.5 trigger fires)**

Omitted entirely when no trigger fires. When present, appears in one of two forms:

- **Trigger 1 (CVE/unmaintained):** Appears as a HOLD finding at the top of the HOLD section, not as a separate STACK section.
- **Trigger 2 (finding traceable to stack bug):** Appears after BEYOND VISUAL, linking specific findings to their root cause and fix.
- **Trigger 3 (user request):** Appears as a full stay + fix assessment.

Never emitted as boilerplate. If you find yourself writing the same STACK note on every review for the same healthy stack, you have misread the trigger conditions.
...
**`.visualreviewignore` generation** — if the user replies "ignore [finding-ID]", generate the entry in this format and instruct them to append it to `.visualreviewignore` at the project root:
...
Valid `finding-type` values: `layout-drift`, `missing-footer`, `missing-eyebrow`, `overflow`, `orphan`, `density-imbalance`, `brand-drift`, `rule-overlap`, `text-inferred`.

If the finding spans multiple pages, use `page:N,M,O` or `page:*` if it applies everywhere. After generating the entry, note: "Add this line to `.visualreviewignore` in your project root. On the next review, this finding will be suppressed and listed in the SUPPRESSED FINDINGS section."

> **Want to act on any of these?**
> - Reply with finding IDs (e.g. "HOLD-1, FIX-3") to receive Phase 9 fix-guidance
> - Reply "export fix-guidance" to receive the full Phase 9 fix-guidance list
> - Reply "ignore [finding-ID]" for any NOTE or possible finding you want suppressed in future reviews — I'll generate the correct `.visualreviewignore` entry for you
> - Reply "output handoff block" to get a YAML block for delta-comparing against the next revision of this document
> - Re-review requires a fresh invocation on the updated source
...
### Session handoff block (opt-in — not output by default)

**When to generate:** Only when the user explicitly requests it:
- They say "output handoff block" or "generate handoff"
- They are reviewing a revised version of the same document (same filename or path appears in conversation context)

**Do NOT output this block by default.** Most users review different documents each time; appending YAML to every report is clutter for them.

When requested, output:

```yaml
# Visual Review — Session Handoff
# Paste this block at the start of your next /visual-review invocation
# on the SAME document to get a delta comparison.

document:
  name: "<document name from report header>"
  page_count: N
  render_timestamp: "YYYY-MM-DD HH:MM"

findings:
  - id: HOLD-1
    status: open       # open | fixed | deferred | wont-fix
    summary: "<one-line summary from report>"
    pages: [N, M]
  - id: HOLD-2
    status: open
    summary: "<summary>"
    pages: [N]
  # ... one entry per HOLD and FIX-IF-TIME finding
```

**On next invocation with a pasted handoff block:**
- Note which findings were marked `fixed` — verify they are resolved in the new render
- Note which were `deferred` or `wont-fix` — do not re-report them unless they worsened
- Add any genuinely new findings not in the prior finding list
- Output a delta summary at the end of Phase 8: "3 prior findings resolved, 1 deferred finding worsened (now HOLD), 2 new findings"
